...
cd build && ctest --config Debug -R "test_name_pattern"
```

Outputs go to `build/bin/` (executables) and `build/lib/` (static libraries). The three executables are `DolasEditor.exe`, `ShaderCompiler.exe`, and `DolasTest.exe`.
