...
float3 ProceduralSkyColor(float2 texcoord)
{
    float t = saturate(texcoord.y);
    float3 zenith = float3(0.10f, 0.34f, 0.64f);
    float3 horizon = float3(0.63f, 0.80f, 0.92f);
    float3 ground = float3(0.08f, 0.18f, 0.10f);

    if (t < 0.58f)
    {
        return lerp(zenith, horizon, t / 0.58f);
    }
    return lerp(horizon, ground, (t - 0.58f) / 0.42f);
}
...
float4 PS(PS_INPUT input) : SV_TARGET0
{
    ...
    if (depth >= 0.999f)
    {
        return float4(ProceduralSkyColor(input.texcoord), 1.0f);
    }
